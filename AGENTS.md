...
7. **Trailing slashes are required and verified for quickshell.org only.** It 308-redirects non-slash doc URLs; all quickshell.org fetches request the slash form directly (`_guide_page`/`_type_page`), and a test asserts the fetched URL shape. doc.qt.io and the Gitea API use their own URL shapes; don't apply the slash rule there.
8. **There is no canonical `/docs/` index; it 404s.** Version discovery scans `/about/` first (it links every published version), then `/docs/`, then `/`. Don't "fix" this back to fetching `/docs/`.
...
nix develop .#ci                             # headless CI env (quickshell, weston, grim, imagemagick)
nix build && nix flake check                 # package + flake evaluation
python scripts/ci_validate.py <project>      # headless QML validation entrypoint
python scripts/quality_audit.py              # per-tool quality audit
python scripts/benchmark.py                  # end-to-end correctness/latency/recovery
```
...
2. **Extraction is per-source.** quickshell.org strips `[class*=sidebar]` junk; doc.qt.io must NOT use that rule because its page body lives in `article.b-sidebar__content...` (see `_QT_STRIP_SELECTORS`). When adding a source, verify extraction against a real snapshot before trusting it.
3. **CI pipeline** (`.github/workflows/ci.yml`): flake check → nix build + dist/twine → lint → offline pytest with coverage → Dockerfile validation; live-smoke runs only on manual dispatch. If you add a check locally, wire the equivalent into CI.
4. **Implementation references are not documentation.** Caelestia/Noctalia/dots-hyprland results carry `kind: real-world implementation` and every tool docstring says the docs win on API disputes. Noctalia's ref is pinned to its `legacy-v4` branch deliberately (its `main` moved to a C++ compositor); a GitHub `truncated: true` tree response is refused rather than silently under-indexed. dots-hyprland's QML root is nested under `dots/.config/quickshell/ii/` and paths are reported relative to it.
5. **In-memory registries are thread-guarded.** `_SESSION_REGISTRY`, `_PROFILE_REGISTRY`, and `_PROJECT_MEMORIES` are guarded by `threading.RLock` because the MCP server can dispatch concurrent requests (HTTP transport); a torn dict read/write must never hand out a half-written entry. Keep new in-memory registries consistent with that.
6. **Semantic release needs an explicit version bump.** `.releaserc.js` matches `Semver:` note trailers (`minor`, `major`, `patch`). A notes-bearing commit with no matching rule falls through to the analyzer's default rules and can be misclassified (a patch was once bumped to major), so release-worthy commits must carry an explicit `Semver:` trailer; there is no reliable implicit patch default.

## ProjectContext (`quickshell_mcp/sources/project.py`)

Shared internal infrastructure (not a capability or MCP tool) for representing a Quickshell project on disk. Consumed via `_build_project_context(path).discover({fields})`. Design rules: never fabricates (missing data is `None`/`[]` with status `"unknown"`), lazy by default, instance- and shared-cached (file-tree scan reuses the 30-minute cache keyed by resolved root), no HTTP (purely local filesystem; version resolution against the live site is the caller's job), compositor-independent (no hardcoded compositor list; derives from `Quickshell.<Name>` imports).

The field table (`qml_files`, `imports`, `compositor`, etc.), each field's detected/inferred/unknown status, the consumer code example, and the steps for adding a new field live in the module's own docstring. Check there before extending it, since that's the copy kept in sync with the code.
...
> A semantic-release CI runs on every merge to `main`. It releases only when a commit carries an explicit `Semver:` trailer. There is no implicit patch default: a notes-bearing commit without a matching `Semver:` rule falls through to the analyzer defaults and can be misclassified (a patch was once bumped to major). Forgetting `Semver: minor` on a new tool means no minor release.
...
- `Semver: patch` (explicitly maps to a patch release)
